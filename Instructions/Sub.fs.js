...
export function subHLpointer(regs, mem) {
...
export function sub8bitRegisters(a, b, regs) {
...
export function subFromMem(regs, memory) {
    const a = tryFind(new GameBoyRegister(0, []), regs);
    ...
    switch (matchResult) {
        case 0: {
            ...
            switch (matchResult_1) {
                case 0: {
                    const res = i - n;
                    const carry = isCarry7to8(i, n);
                    const half = isHalfCarry3to4(i, n);
                    const z = isZero(res);
                    const f = new Flags(z, true, half, carry);
...
export function filterSub(r1, r2, reg, mem) {
    let matchResult, n;
    switch (r2.tag) {
        case 3: {
    ...
    switch (matchResult) {
        case 0:
            return subHLpointer(reg, mem);
        case 1:
            return subFromMem(reg, mem);
        case 2:
            return sub8bitRegisters(new GameBoyRegister(0, []), n, reg);
        default:
            return undefined;
    }
}
