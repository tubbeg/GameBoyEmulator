...
export function addCHLpointer(regs, mem) {
    ...
    if (hl != null) {
        ...
        switch (matchResult) {
            case 0: {
                ...
                const ur = add(new GameBoyRegister(0, []), new BitData(1, [setMax(res, carry)]), regs);
                return [ur, f];
            }
            default: {
...
export function addC8bitRegisters(a, b, regs) {
    ...
    switch (matchResult) {
        case 0: {
            ...
            const z = isZero(res);
            const f = new Flags(z, false, half, carry);
            const updatedRegisters = add(a, new BitData(1, [setMax(res, carry)]), regs);
...
export function addCFromMemory(regs, memory) {
    const a = tryFind(new GameBoyRegister(0, []), regs);
        ...
        case 0: {
            ...
            switch (matchResult_1) {
                case 0: {
                    ...
                    const z = isZero(res);
                    const f = new Flags(z, false, half, carry);
                    const updatedRegs = add(new GameBoyRegister(8, []), new BitData(0, [adress + 1]), add(new GameBoyRegister(0, []), new BitData(1, [setMax(res, carry)]), regs));
...
export function filterAddC(r1, r2, reg, mem) {
    let matchResult, n;
    switch (r2.tag) {
        case 3: {
    ...
    switch (matchResult) {
        case 0:
            return addCHLpointer(reg, mem);
        case 1:
            return addCFromMemory(reg, mem);
        case 2:
            return addC8bitRegisters(new GameBoyRegister(0, []), n, reg);
        default:
            return undefined;
    }
}
