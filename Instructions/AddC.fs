...
let addCHLpointer   (regs : RegisterMap) (mem : MemoryBus)  =
    ...
    | Some adress ->
        ...
        | Some(dataHL), Some(B8 b) ->
            ...
            let ur = regs |> Map.add A (B8 (setMax res carry))
            Some(ur,f)
        | _ ->
...
let addC8bitRegisters  a b (regs : RegisterMap) :( RegisterMap * Flags) option  =
    ...
    | Some(B8 x),Some(B8 y) -> 
        ...
        let z = isZero (uint res)
        let f = {Zero=z;Sub=false;HalfCarry=half;Carry=carry}
        let updatedRegisters = regs |> Map.add a (B8 (setMax res carry))
        Some(updatedRegisters,f)
    | _ ->
        printfn "No 8 bit values detected!"
        None

let addCFromMemory (regs : RegisterMap) (memory : MemoryBus) =
    let a = regs |> Map.tryFind A
    ...
    | Some (B16 adress) ->
        ...
        | Some n, Some (B8 i) ->
            ...
            let f = {Zero=z;Sub=false;HalfCarry=half;Carry=carry}
            let updatedRegs =
                regs |> Map.add A (B8 (setMax res carry)) // update result in registers
...
let filterAddC (r1,(r2 : Resource)) reg mem =
    match r2 with
    | Pointer(Vregister HL) -> addCHLpointer reg mem
    | Number N8 -> addCFromMemory reg mem
    | Register n -> addC8bitRegisters A n reg
    | _ -> None