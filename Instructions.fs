...
let InstructionOpCodeMap : Map<byte,Instruction> =
    ...
    //XOR
    |> Map.add 0xA8uy (XOR (Register A,Register B))
    |> Map.add 0xA9uy (XOR (Register A,Register C))
    |> Map.add 0xAAuy (XOR (Register A,Register D))
    |> Map.add 0xABuy (XOR (Register A,Register E))
    |> Map.add 0xACuy (XOR (Register A,Register H))
    |> Map.add 0xADuy (XOR (Register A,Register L))
